  ...
  const stars = [
    ...
    citiesList.forEach((city) => {
      describe(`Test filters for ${city.name}`, () => {
        beforeEach(() => {
          cy.visit(`https://wshotfix.ihogroup.ir${city.link}`);
          cy.get("h1")
            .should("exist")
        ...
        it.only('should check all stars filter',()=>{
          let count = 0;
          cy.get('#filters-sidebar iho-stars-filter ul li').each(($el)=>{
            cy.wrap($el).within(()=>{
              cy.get('label').click({timeout: 60000}).then(()=>{
                cy.document().find('h1')
                  .should("exist")
                  count++
              })
              ...
              if(count === 1){
                let grade = cy.location('search').invoke('charAt',-1)
                cy.log('here')
                cy.log(grade)
                cy.get('h1').should('exist').and('include',`هتل های ${city.name}`)
              }
              if(count >= 2){
                cy.get('h1').should('exist').and('contain',`هتل های ${city.name}`)
              } 
            })
